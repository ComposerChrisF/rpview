...
- [x] **GPU Lanczos resize** — separable Lanczos-3 in `shaders/lanczos_h.wgsl` + `shaders/lanczos_v_oklab.wgsl`.  H pass: `Rgba8UnormSrgb` source (linear via hardware on `textureLoad`) → `Rgba16Float` linear intermediate at `(out_w, src_h)`.  V pass: linear intermediate → `Rgba16Float` OKLab `buf_a`, folding `linear_srgb_to_oklab` into the second sweep so the resize path skips the standalone `decode_oklab` dispatch.  Filter scale widens the kernel for downscaling (`max(1, src/dst)` per axis).  CPU `resize_rgba8_bilinear` removed; `cache.rs` split into source / intermediate / output LRUs (each capacity 4) so resize-factor scrubbing on one image doesn’t duplicate the source texture

#### Polish
- [x] **Persisted GPU Pipeline window bounds + open state** — `Appearance.gpu_pipeline_window_bounds` + `gpu_pipeline_window_open` mirror the CPU LC pattern.  `open_gpu_pipeline_window` reads bounds from settings (falling back to centered 340×620), `observe_window_bounds` saves on move/resize via `save_settings_debounced`, `on_release` clears the open flag, and `main.rs` reopens the window on launch when it was open at last quit
- [x] **CPU LC fallback save support** — audited the unified save path against the renderer’s priority chain.  `lc_active` detection covers both static `lc_render` and per-frame `lc_frame_renders[current]`; `capture_current_display` returns the correct render (per-frame preferred for animated, `lc_render` fallback for static); per-frame `RenderImage` construction is identical to static so `as_bytes(0)` yields the correct bytes; `snapshot.width/height` come from `lc_render_size` or the per-frame size tuple so resize-factor LC saves at the right dimensions.  No code changes needed
- [x] **Presets** — `utils/gpu_presets.rs` mirrors `utils/lc_presets.rs`: JSON files in `{settings_dir}/gpu-presets/`, `GpuPreset` struct captures the full panel state (per-stage enable flags + every slider value + resize factor) so a load restores the user’s exact UI configuration including slider values for stages that happen to be disabled at save time.  GPU Pipeline panel grew a Presets section at the top with dropdown, name input + Save/Del buttons — same layout as the LC panel.  10 unit tests cover round-trip, sanitization, sorting, eviction, and the `(Custom)` sentinel
